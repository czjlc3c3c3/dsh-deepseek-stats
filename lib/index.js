...
const BUILTIN_PRICES = {
  ...
  // 实验模型：官方价目页未列出，当前与 flash 同价（用户确认 2026-09）
...
export function apply(ctx) {
  ...
  function priceOf(model) {
    ...
    // 宽松兜底：flash 系列（含 v4.1-flash-*、*-flash-exp 等实验/新变体）按 flash 计价
    if (m.indexOf('flash') >= 0) return activePrices['deepseek-v4-flash']
    return null
  }
  ...
  async function syncPrices() {
    try {
      ...
      // 官方页只列官方模型；内置表中官方页没有的（如实验模型）继续用内置价，不被整体覆盖丢掉
      const merged = {}
