...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function WindowRing(props) {
      ...
      var dayTip = w && w.dayType === 'holiday' ? '今天是中国法定节假日（' + (w.holidayName || '节假日') + '），全天按空闲 ×1 计费。'
        : (w && w.dayType === 'makeup' ? '今天是调休上班日（周末），官方规则下仍按空闲 ×1 计费。'
        : (w && w.dayType === 'weekend' ? '今天是周末，全天按空闲 ×1 计费。' : ''));
      var title = w ? ('DeepSeek 峰谷计价：北京时间 周一至周五（不含中国法定节假日）09:00-12:00、14:00-18:00 为高峰时段，价格为空闲价 ×' + factor + '；其余时间（含周末、法定节假日全天、夜间）空闲 ×1。' + dayTip + '当前 ' + (isPeak ? '高峰（×' + factor + ' 价）' : '空闲（×1 价）') + ' · 本时段剩余 ' + Math.round(frac * 100) + '%（' + fmtDur(remainMs) + '）') : '计算时段…';
    ...
    function Widget() {
      ...
      var children = [head];
      if (data) {
        var holiday = data.holiday || null;
        var priceNote = null;
        if (price) {
          var holidayTag = holiday ? ' · 节假日源 ' + (holiday.source === 'sync' ? '官方数据集' : '内置') + (holiday.years && holiday.years.length ? '（' + holiday.years.join('/') + '）' : '') : '';
          if (price.source === 'official') {
            priceNote = '价源 官方（' + fmtShort(price.at) + ' 同步 · 高峰 ×' + price.factor + ' · 每 6h 自动同步）' + holidayTag;
          } else {
            priceNote = '价源 内置（官方价格页同步失败' + (price.error ? '：' + String(price.error).slice(0, 80) : '') + '）' + holidayTag;
          }
        }
        var body = React.createElement('div', { className: 'dsbal-body' },
          ...
          React.createElement('div', { className: 'dsbal-note' }, (function () {
            if (!w) return '计算时段…';
            if (w.isPeak) return '当前 高峰时段（价格×' + (w.factor || 2) + '）· 空闲 ' + (w.nextLabel || '') + ' 开始（' + fmtDur(w.msUntil) + ' 后）';
            var idleWhy = w.dayType === 'holiday' ? '法定节假日' + (w.holidayName ? '（' + w.holidayName + '）' : '') + ' · 全天 '
              : (w.dayType === 'makeup' ? '调休上班日 · 仍按 ' : (w.dayType === 'weekend' ? '周末 · 全天 ' : ''));
            var next = w.nextLabel ? '高峰 ' + w.nextLabel + ' 开始（' + fmtDur(w.msUntil) + ' 后）' : '暂无高峰时段';
            return '当前 空闲时段（价格×1）' + (idleWhy ? ' · ' + idleWhy : ' · ') + next;
          })()),
          priceNote ? React.createElement('div', { className: 'dsbal-note' }, priceNote) : null,
          (function () {
            var bf = data.backfill;
            if (!bf || !bf.at) return null;
            var how = bf.mode === 'mtime' ? '今日活跃会话' : (bf.mode === 'all' ? '全量会话' : '未回填');
            var txt = '回填 ' + how + ' ' + (bf.candidates || 0) + '/' + (bf.sessions || 0) + ' 个 · 成功 ' + (bf.scanned || 0) + ' · 折叠 ' + (bf.events || 0) + ' 条';
            if (bf.mode === 'all') txt += '（未识别会话存储布局，已全量兜底）';
            return React.createElement('div', { className: 'dsbal-note' }, txt);
          })(),
          React.createElement('div', { className: 'dsbal-foot' },
            React.createElement('span', null, '今日总计 ' + fmtTokens(totals ? totals.tokens : null) + ' tokens（全部署）'),
