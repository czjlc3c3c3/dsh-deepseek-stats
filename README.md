...
- **峰谷计价**：高峰时段 = 北京 周一至周五 09:00–12:00 / 14:00–18:00（价格 ×官方倍率，默认 2），其余时间（含周末、夜间）×1；按每条消息发起时刻计价；缓存命中 / 未命中 / 缓存写 / 输出分档；未知模型不计价并在界面标注。内置价目含实验模型 `deepseek-v4.1-flash-expires-on-0910`（官方页未列出，与 `deepseek-v4-flash` 同价）；名称含 `flash` 的未知变体按 flash 价兜底。
